...
from database_setup import Base, Categories, CatalogItems, User
    ...
    if request.method == 'POST':
        ...
        if request.form['category']:
            category = session.query(Categories).filter_by(
                name=request.form['category']).one()
            editItem.categories_id = category.id
...
def JSONPage():
    ...
    for catItem in range(len(catJSON)):
        itemJSON = [j.serialize for j in session.query(CatalogItems).filter_by(
            categories_id=catJSON[catItem]["id"]).all()]
        if itemJSON:
            catJSON[catItem]["Item"] = itemJSON
    return jsonify(Category=catJSON)
